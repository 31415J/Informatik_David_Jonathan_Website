# Informatik_David_Jonathan_Website
The Website that we develop for IT-lessons in school

What needs to be done:
- more pages (for example: what is blender, some projects, renderings, links to blender and some tutorials)
- mobile devices optimisation
- auto switch light and dark mode option
- Content
- 

In this Website, we try to format all variables, id, name as Kebab Case. That meens you connect two word with a hyphen(-). For example content-gallery-box-image-border.




This was mainly tested on a Mac with MacOS 26.5 with Firefox, Safari, Google Chrome, (DuckDuckGo didn't worked because linking the css failed). The Website is made for an screen in Full HD resolution (work also for different devices, but it should work best on those.)